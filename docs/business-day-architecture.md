...
- **Trading Period Ended ≠ Business Day Closed.** `phase == CLOSED` blocks *trading only* — `openSession` and checkout. Session closure, `begin-closure`, the X-Report and Day Close/Z-Report are deliberately ungated and must never wait for the next window; that separation is what lets the operator finish yesterday's day at 23:30. The POS overlay reflects it: with `sessionsRequiringClosure` non-empty it reads **"Trading Period Ended"** and gives every listed session (all terminals, from backend state — not just the one this browser resolved) its own Close Session action; only with the list empty does it say "Business Day Closed". The overlay is blocking, so it stands down entirely while a closure flow is on screen rather than covering the denomination UI.
- **Every post-trading state is actionable** — the CLOSED phase covers three distinct states, and none may be a dead end, because the operator can re-enter the POS at any point (browser Back out of the X-Report, a refresh, a second browser) and must still be able to finish: `sessionsRequiringClosure` non-empty → *Trading Period Ended*, Close Session per session; `dayCloseRequired` → *Day Close Required*, one **Open Day Close** button into the existing Day Close/Z-Report view; neither → *Business Day Closed*, informational. **"Trading ended" is never equivalent to "Business Day finalized."** `dayCloseRequired` is computed on Day Status as `phase == CLOSED && sessionsRequiringClosure.isEmpty() && (thisDateUnfinalized || earlierDatePending)`, where finalization is read from the `pos_day_close` records only — **never** from `phase == CLOSED`, from zero open sessions, or from completed X-Reports, all of which merely say session closure progressed. Two authoritative reads: `existsByBranchIdAndCloseDate(branch, tradingDate)` answers the direct question and still holds when a *later* date was closed first (which pushes `PosPendingDayCloseResolver`'s last-closed anchor past this date, so the resolver alone would report nothing pending while this date has no record); and the resolver's own pending date `<= tradingDate` catches an *earlier* date still unclosed. A Trading Date with no sessions at all is exempt — it was never owed a Day Close, the same rule that lets the resolver ignore empty calendar dates. Backend-derived like the session list, so a second browser refreshing sees the same next step. It is a navigation signal only: the Day Close flow still enforces its own validations (X-Reports complete, no open sessions or suspended bills, supervisor approval), and nothing here reopens a finalized date.
