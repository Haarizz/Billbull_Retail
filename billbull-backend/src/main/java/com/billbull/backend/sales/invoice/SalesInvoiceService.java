...
public class SalesInvoiceService {
    ...
    public void updateStatus(Long id, SalesInvoiceStatus status) {
        invoiceRepo.findById(id).ifPresent(invoice -> {
            ...
            if (isNewlyPosted) {
                ...
                if (!isLinkedToDn) {
                    boolean autoDelivered = autoGenerateDeliveryNote(invoice);
     ...
     */
    private boolean autoGenerateDeliveryNote(SalesInvoice invoice) {
        ...
        // QA-008 FIX: For Direct Sales, goods are handed over at the point of sale.
        // Immediately advance the DN to DELIVERED to deduct stock automatically.
        if (invoice.getSalesType() == SalesType.DIRECT_SALE) {
            ...
            System.out.println("[Auto-DN] Direct Sale: DN " + created.dnNumber + " auto-delivered, stock deducted.");
            return true;
        }

        // Non-direct-sale DN stays in DRAFT — stock deducted only when manually marked DELIVERED
        return false;
    }
