...
public class SalesInvoiceService {
    ...
    public SalesInvoice save(SalesInvoice invoice) {
        ...
        // themselves are emitted after the save succeeds.
        final boolean historyIsCreate = existing == null;
        final java.util.List<String> historyChanges = historyIsCreate
                ? java.util.List.of()
                : historyService.diff(existing, invoice);
        final String priorLinkedQuotation = existing == null ? null : existing.getLinkedQuotation();
        final String priorLinkedProforma = existing == null ? null : existing.getLinkedProforma();
        final String priorLinkedSalesOrder = existing == null ? null : existing.getLinkedSalesOrder();
        final String priorLinkedDeliveryNote = existing == null ? null : existing.getLinkedDeliveryNote();
        ...
        // never roll back the invoice).
        if (historyIsCreate) {
            historyService.recordCreated(saved);
        } else {
            historyService.recordUpdated(saved, historyChanges);
        }
        recordNewLinkEvents(saved, priorLinkedQuotation, priorLinkedProforma,
                priorLinkedSalesOrder, priorLinkedDeliveryNote);
    ...
    /**
     * Activity timeline for one invoice: stored events plus events derived from the
     * invoice's own columns (so invoices predating the history table aren't blank).
     * Routed through getById so it inherits the same not-found + branch-access guards.
     */
    @Transactional(readOnly = true)
    public java.util.List<com.billbull.backend.sales.invoice.history.SalesInvoiceHistoryResponse> getHistory(Long id) {
        return historyService.getHistory(getById(id));
    }
     ...
     * inside an active session, or serialization fails with LazyInitializationException.
     */
    @Transactional(readOnly = true)
    ...
    private void doUpdateStatus(Long id, SalesInvoiceStatus status, SalesSettings settings) {
        invoiceRepo.findById(id).ifPresent(invoice -> {
            ...
            // History: record the transition (no-ops are filtered inside the service).
            historyService.recordStatusChanged(invoice,
    ...
    /**
     * Emits a LINKED_DOCUMENT history event for each linked-document field that went from
     * blank to set on this save. These four fields are never assigned by this service —
     * they arrive on the frontend payload — so a null -> value transition is the only
     * moment we can honestly timestamp the link being made.
     */
    private void recordNewLinkEvents(SalesInvoice saved, String priorQuotation, String priorProforma,
            String priorSalesOrder, String priorDeliveryNote) {
        recordLinkEventIfNew(saved, "QUOTATION", priorQuotation, saved.getLinkedQuotation());
        recordLinkEventIfNew(saved, "PROFORMA", priorProforma, saved.getLinkedProforma());
        recordLinkEventIfNew(saved, "SALES_ORDER", priorSalesOrder, saved.getLinkedSalesOrder());
        recordLinkEventIfNew(saved, "DELIVERY_NOTE", priorDeliveryNote, saved.getLinkedDeliveryNote());
    }

    private void recordLinkEventIfNew(SalesInvoice saved, String documentType, String before, String after) {
        if (after == null || after.isBlank() || java.util.Objects.equals(before, after)) {
            return;
        }
        historyService.recordLinkedDocument(saved, documentType, after);
    }
