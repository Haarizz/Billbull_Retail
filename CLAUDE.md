...
- Schema is JPA-managed (`spring.jpa.hibernate.ddl-auto=update`) **and** Flyway is active (`spring.flyway.enabled=true`, `baseline-on-migrate=true`, `baseline-version=1`, scripts in `src/main/resources/db/migration`). The two coexist deliberately: Flyway scripts are additive/idempotent (guarded with `to_regclass` checks) and run before Hibernate on every boot; `ddl-auto` stays `update` until each tenant DB is baselined, then flips to `validate` per-tenant. 96 migrations exist (`V1__baseline.sql` through `V100__product_price_change_audit.sql`; version numbers V2/V4/V5/V98 are retired and permanently skipped). Gaps are expected and must be left alone: `spring.flyway.out-of-order` is unset (Flyway's default is `false`), so renumbering a script down into a gap makes it out-of-order on every tenant that already ran the higher version and fails the migration on boot — see the note at the top of `V8__fk_constraints.sql`. Always take the next free number above the highest applied one; never reuse a retired number. New schema changes should generally get a new Flyway migration rather than relying solely on Hibernate auto-DDL, especially for anything touching existing client data.
...
- The unit suite (Mockito tests) is fully green. Seven `@SpringBootTest` classes need a reachable DB and fail context load on a bare `mvn test` when no datasource is configured — `BillbullBackendApplicationTests`, `DiagnosticTest`, `CostCenterBranchTest`, `CostCenterControllerTest`, `ProductPriceChangeRepositoryQueryTest`, `PosStockDisplayRuntimeAuditTest` and `E2ESameBranchFallbackTest`. Pass `-Dspring.datasource.url/username/password` or activate a profile. None is a code regression.
- There is no failsafe plugin, so a class named `*IT` is never executed by any build phase. Name a DB-backed test `*Test` or it silently never runs.
