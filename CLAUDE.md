...
- The unit suite (Mockito tests) is fully green. Two tests need a reachable DB and fail on a bare `mvn test` when no datasource is configured — `BillbullBackendApplicationTests.contextLoads` and `ProductPriceChangeRepositoryQueryTest` (both `@SpringBootTest`). Pass `-Dspring.datasource.url/username/password` or activate a profile. Neither is a code regression.
