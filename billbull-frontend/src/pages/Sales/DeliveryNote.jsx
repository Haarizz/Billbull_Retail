...
const DeliveryNote = () => {
    ...
    const [dnDate, setDnDate] = useState(toLocalInputDate());

    // Dispatch date: defaults to today and can never be back-dated. An already
    // saved draft that carries an older date keeps that date as its floor, so
    // reopening it does not force the user to change a legitimate past value.
    const todayInputDate = toLocalInputDate();
    const minDispatchDate = dnDate && dnDate < todayInputDate ? dnDate : todayInputDate;
    const handleDnDateChange = (value) => {
        if (!value) return;                     // required field — ignore a clear
        setDnDate(value < minDispatchDate ? minDispatchDate : value);
    };
    ...
    // VAT mode for any manually-entered/edited line (linked SO/Invoice lines
    // already arrive with a pre-computed taxAmt/netAmount and bypass this).
    ...
    const normalizeDeliveryItem = (item = {}, fallbackId = Date.now() + Math.random()) => {
        ...
        // No pre-computed tax/total on the payload — derive both via the
        // shared VAT helper so an Inclusive-mode raw price isn't mistaken
        // for an already ex-VAT amount.
        const derived = computeLineTaxTotals({ netAfterDiscount, taxPercent: tax, vatMode });
        const taxableAmount  = derived.taxableAmount;
        const taxAmt         = Number.isFinite(taxAmountFromPayload) ? taxAmountFromPayload : derived.taxAmount;
        const total          = Number.isFinite(netAmountFromPayload) ? netAmountFromPayload : derived.total;
    ...
    const handleRowClick = (dn) => {
        ...
        setVatMode(dn.vatMode === 'INCLUSIVE' || dn.taxInclusive ? 'INCLUSIVE' : 'EXCLUSIVE');
    ...
    const recomputeItemTotals = (item) => {
        ...
        const discountAmount = grossAmount * (disc / 100);
        const netAfterDiscount = grossAmount - discountAmount;
        const { taxableAmount, taxAmount: taxAmt, total } = computeLineTaxTotals({
            netAfterDiscount,
            taxPercent: tax,
            vatMode,
        });
        ...
        setItems(items.map(item => {
            if (item.id === id) {
                ...
                if (field === 'currentQty') {
                    ...
                    // A Delivery Note only ever dispatches stock out, so a negative
                    // "To Deliver" is never valid — clamp it to 0 instead of letting
                    // it flow into totals, picking and the stock movement payload.
                    const qty = !Number.isFinite(parsed) || parsed < 0 ? 0 : parsed;
    ...
    const handleSave = async (shouldRedirect = true) => {
        ...
        // Quantity validation — negative "To Deliver" must never reach the backend.
        const negativeLine = validItems.find(i => Number(i.currentQty) < 0 || Number(i.foc) < 0);
        if (negativeLine) {
            alert(`Delivery quantity cannot be negative for ${negativeLine.code || negativeLine.desc || 'item'}.`);
            return null;
        }

        if (!validItems.some(i => Number(i.currentQty) > 0 || Number(i.foc) > 0)) {
            alert("Enter a delivery quantity greater than 0 for at least one item.");
            return null;
        }
                        ...
                        <div className="space-y-6 flex-1 flex flex-col pb-24 animate-in fade-in duration-200">
                            ...
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-3 bg-white rounded-lg border border-slate-200 p-4">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <input disabled={isLockedForEdit} type="date" value={dnDate} min={minDispatchDate} onChange={e => handleDnDateChange(e.target.value)} className="w-32 text-sm p-1.5 border border-slate-200 rounded text-slate-700 disabled:bg-slate-50 disabled:text-slate-400 disabled:cursor-not-allowed" />
                                </div>
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-4 gap-4 xl:gap-6 items-start">
                                ...
                                <div className="xl:col-span-3 space-y-4">
                                    ...
                                    <CustomerShippingPanel
                                        ...
                                        expectedDispatch={dnDate}
                                        onExpectedDispatchChange={handleDnDateChange}
                                        minExpectedDispatch={minDispatchDate}
                                        isReadOnly={isLockedForEdit}
                                        currency={currency}
                                    />
                                    </div>
