...
import {
    RefreshCw, Download,
    TrendingDown, DollarSign, ShoppingCart, Calendar,
    Menu, X
} from 'lucide-react';
...
const PurchaseSummaryReport = () => {
    ...
    const filtered = useMemo(() => {
        let data = invoices;
        if (dateFrom) data = data.filter(i => (i.invoiceDate || i.date || '') >= dateFrom);
        if (dateTo) data = data.filter(i => (i.invoiceDate || i.date || '') <= dateTo);
        if (statusFilter !== 'All') data = data.filter(i => (i.status || '').toUpperCase() === statusFilter.toUpperCase());
        if (vendorFilter !== 'All') data = data.filter(i => i.vendorName === vendorFilter);
        if (searchQuery) {
        ...
        return data;
    }, [invoices, dateFrom, dateTo, statusFilter, vendorFilter, searchQuery]);

    const vendors = useMemo(() => ['All', ...new Set(invoices.map(i => i.vendorName).filter(Boolean)).values()], [invoices]);
    ...
    const groupedData = useMemo(() => {
        ...
        filtered.forEach(inv => {
            const key = groupBy === 'Vendor' ? (inv.vendorName || 'Unknown') :
                groupBy === 'Month' ? (inv.invoiceDate || inv.date || '').substring(0, 7) :
                    (inv.status || 'Unknown');
            if (!map[key]) map[key] = { name: key, count: 0, total: 0, paid: 0 };
            map[key].count += 1;
            map[key].total += parseFloat(inv.grandTotal || inv.totalAmount || 0);
            if ((inv.status || '').toUpperCase() === 'PAID') map[key].paid += parseFloat(inv.grandTotal || inv.totalAmount || 0);
        });
    ...
    const handleExport = () => {
        ...
        a.download = `purchase_summary_${dateFrom}_to_${dateTo}.csv`; a.click();
    };
        ...
        <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
            ...
            `}>
                <div className="p-4 border-b border-slate-100">
                    <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                            <h2 className="m-0 text-base font-bold text-slate-900">Purchase Summary</h2>
                            <p className="mt-0.5 text-xs text-slate-500">Procurement overview, vendor analysis, and invoice breakdown.</p>
                        </div>
                        <button
                            type="button"
                            onClick={() => setSidebarOpen(false)}
                ...
                <div className="flex-1 overflow-y-auto p-3">
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '0 4px' }}>
                        ...
                        <div>
                            <label style={{ fontSize: 11, fontWeight: 600, color: '#6b7280', display: 'block', marginBottom: 4 }}>Vendor</label>
            ...
            <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
                ...
                <div className="md:hidden flex items-center gap-3 px-4 py-3 bg-white border-b border-slate-200 shrink-0">
                    ...
                    <div className="min-w-0">
                        <div className="text-sm font-bold text-slate-900 truncate">Purchase Summary</div>
