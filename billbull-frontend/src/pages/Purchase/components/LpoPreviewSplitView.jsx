...
import LpoCardList from './LpoCardList';
import LpoPreview from './LpoPreview';

// LPO Transaction Preview page: persistent LPO switcher + read-only preview.
// Mirrors the sales split-views. LPOs are keyed by lpoNumber (their id).
export default function LpoPreviewSplitView({
    lpos,
    previewLpoNumber,
    onSelectLpo,
    listLoading,
    ...
    const selectedLpo = lpos?.find((l) => l.lpoNumber === previewLpoNumber);

    const handleSelect = (lpo) => {
        onSelectLpo(lpo);
        setDrawerOpen(false);
    };
    ...
    const listNode = (
        <LpoCardList
            lpos={lpos}
            selectedNumber={previewLpoNumber}
            onSelect={handleSelect}
            currency={previewProps.lpoCurrency}
    ...
    return (
        <div className="animate-in fade-in duration-200">
            ...
            >
                ...
                Browse LPOs
                {selectedLpo && <span className="text-slate-400 font-normal">· {selectedLpo.lpoNumber}</span>}
            </button>
            ...
            <div className="grid lg:grid-cols-[minmax(260px,300px)_1fr] xl:grid-cols-[minmax(300px,340px)_1fr] gap-4 items-start">
                ...
                <div className="min-w-0">
                    <LpoPreview lpoNumber={previewLpoNumber} {...previewProps} />
                </div>
            </div>
                ...
                <div className="lg:hidden fixed inset-0 z-50 flex">
                    ...
                    >
                        <div className="flex items-center justify-between px-3 py-2.5 border-b border-slate-200 bg-white shrink-0">
                            <span className="text-xs font-bold text-slate-700 uppercase tracking-wide">Purchase Orders</span>
                            <button
                                type="button"
                                onClick={() => setDrawerOpen(false)}
                                aria-label="Close"
