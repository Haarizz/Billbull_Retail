...
import LpoPreviewSplitView from '../components/LpoPreviewSplitView';
...
const LPOList = () => {
  ...
  // Transaction Preview (read-only) — selected LPO number for the preview tab.
  const [previewLpoNumber, setPreviewLpoNumber] = useState(null);
  ...
  // Open the read-only Transaction Preview for an LPO (row click / preview tab).
  const openLpoPreview = (lpo) => {
    setPreviewLpoNumber(lpo.lpoNumber || lpo.id);
      ...
      <main className="flex-1 h-full flex flex-col min-w-0 min-h-0 overflow-hidden">
        ...
        <div className="bg-white border-b border-slate-200 px-4 md:px-6 py-5 sticky top-0 z-40 shadow-sm mb-6">
          ...
          <div className="flex overflow-x-auto no-scrollbar gap-2 mb-4">
            ...
            {/* Transaction Preview tab — shown once an LPO is selected for preview */}
            {(previewLpoNumber || activeNavTab === 'preview') && (
              <button
        ...
        {loading && activeNavTab !== 'editor' ? (
          ...
          <>
            ...
            {activeNavTab === 'preview' && (
              <LpoPreviewSplitView
                lpos={visibleRows}
                previewLpoNumber={previewLpoNumber}
                onSelectLpo={(lpo) => setPreviewLpoNumber(lpo.lpoNumber)}
                listLoading={loadingPage}
                searchTerm={searchQuery}
                onSearchChange={setSearchQuery}
                vendorsList={vendors}
                lpoCurrency={currencyLabel}
                isPrinting={loading}
                onBack={() => setActiveNavTab('list')}
                onEdit={(raw) => handleEditLPO({ ...raw, lpoNumber: raw.lpoNumber || raw.id })}
                onPrint={(raw) => handlePrintLPO({ ...raw, lpoNumber: raw.lpoNumber || raw.id })}
                onRecordAdvance={(raw) => handleOpenAdvancePaymentModal({ ...raw, dbId: raw.dbId ?? raw.id })}
                onPrintVoucher={(receipt, raw) => handlePrintPaymentVoucher({ ...raw, dbId: raw.dbId ?? raw.id, lpoNumber: raw.lpoNumber })}
              />
            )}
            {activeNavTab === 'editor' && (
              <EditorView
